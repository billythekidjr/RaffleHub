...
// Safely access globally injected variables to prevent build errors.
...
// --- Stripe Components will be loaded dynamically ---
let Elements, CardElement, useStripe, useElements;

// --- Stripe Payment Component ---
const PaymentModal = ({ raffle, onClose }) => {
    const [stripePromise, setStripePromise] = useState(null);
    const [stripeReady, setStripeReady] = useState(false);

    // This useEffect hook handles the dynamic loading of Stripe scripts.
    useEffect(() => {
        if (window.Stripe && window.ReactStripe) {
            const { loadStripe } = window.ReactStripe;
        ...
        scriptStripe.onload = () => {
            const scriptReactStripe = document.createElement('script');
            scriptReactStripe.src = 'https://unpkg.com/@stripe/react-stripe-js@2.7.1/dist/react-stripe.umd.js';
            scriptReactStripe.async = true;
            
            scriptReactStripe.onload = () => {
                const { loadStripe: loadStripeFn, Elements: Elem, CardElement: Card, useStripe: uStripe, useElements: uElem } = window.ReactStripe;
                Elements = Elem;
                CardElement = Card;
                useStripe = uStripe;
                useElements = uElem;
                ...
                if (stripeKey) {
                    setStripePromise(loadStripeFn(stripeKey));
                    setStripeReady(true);
                }
            };
            document.body.appendChild(scriptReactStripe);
        };
    }, []);

    // The actual checkout form, nested to ensure Stripe is loaded.
    const CheckoutForm = () => {
        ...
        const ticketPrice = parseFloat(raffle?.ticketPrice || 0);
        ...
        const handleSubmit = async (event) => {
            ...
            if (raffle && user) {
                const raffleCollectionRef = collection(db, 'artifacts', appId, 'public', 'data', 'raffles');
                const updatedEntries = [...raffle.entries, { name: user.email, id: crypto.randomUUID(), userId: user.uid }];
                const raffleDocRef = doc(raffleCollectionRef, raffle.id);
                try {
        ...
        return (
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="p-4 bg-gray-700 rounded-lg">
                    {CardElement && <CardElement options={{ style: { base: { color: '#FFFFFF' } } }} />}
                </div>
                {error && <div className="text-red-400 text-sm">{error}</div>}
                <button type="submit" disabled={!stripe || processing} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-lg">
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
            <div className="bg-gray-800 p-8 rounded-xl shadow-2xl w-full max-w-md relative">
                ...
                <h2 className="text-2xl font-bold text-center mb-4 text-purple-300">Buy Ticket for "{raffle?.name}"</h2>
                {stripeReady && stripePromise && Elements ? (
...
function App() {
    ...
    useEffect(() => {
        if (!auth) {
            console.error("Firebase is not initialized. App cannot function.");
            setLoading(false);
            return;
        }
            ...
            <div className="bg-gray-800 p-6 rounded-xl shadow-lg">
                ...
                {raffle.winner ? (
                    ...
                    <>
                        ...
                        <div className="mt-4">
                            <button 
                                onClick={() => openPaymentModal(raffle)} 
                                className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-lg disabled:bg-gray-500 disabled:cursor-not-allowed"
                                disabled={!stripePublishableKey}
                            >
                                {stripePublishableKey ? 'Buy Ticket' : 'Payments Disabled'}
                            </button>
                        </div>
