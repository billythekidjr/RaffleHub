...
try {
    if (window && typeof window.__firebase_config !== 'undefined' && window.__firebase_config) {
        firebaseConfig = JSON.parse(window.__firebase_config);
    }
    if (window && typeof window.__app_id !== 'undefined' && window.__app_id) {
        appId = window.__app_id;
    }
} catch (e) {
    console.error("Error parsing environment configuration:", e);
}
...
// This self-contained component handles all Stripe logic, including script loading.
const PaymentModal = ({ raffle, onClose }) => {
    const [stripePromise, setStripePromise] = useState(null);
    const [stripeReady, setStripeReady] = useState(false);

    // Dynamically load Stripe scripts
    useEffect(() => {
        if (window.Stripe && window.ReactStripe) {
            const stripeKey = process.env.REACT_APP_STRIPE_PUBLISHABLE_KEY || 'pk_test_51Hh2YBEgC0X0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0g0';
            setStripePromise(window.Stripe(stripeKey));
            setStripeReady(true);
            return;
        }
    ...
    const CheckoutForm = () => {
        const { Elements, CardElement, useStripe, useElements } = window.ReactStripe;
